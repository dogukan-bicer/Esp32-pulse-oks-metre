...
float PulseOximeter::getbeat()
{
   // return beatDetector.beatPeriod;
...
//         float irACValue_ = irDCRemover.step(rawIRValue);
//         float redACValue_ = redDCRemover.step(rawRedValue);

//         // The signal fed to the beat detector is mirrored since the cleanest monotonic spike is below zero
//         float filteredPulseValue = lpf.step(_irACValue);
//         return filteredPulseValue;
//     }
      float beat__= filteredPulseValue;
      return  beat__;
}
